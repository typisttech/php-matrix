...
class Application extends SymfonyConsoleApplication
{
    ...
    public const string BUILD_TIMESTAMP = '@datetime@';
    ...
    public function getLongVersion(): string
    {
        ...
        $app = sprintf('%-15s <info>%s</info> %s', $this->getName(), $this->getVersion(), self::BUILD_TIMESTAMP);
        $longVersion .= $app;
        ...
        if ((bool) preg_match('/^(?<tag>.+)-\d+-g(?<hash>[a-f0-9]{7})$/', $this->getVersion(), $matches)) {
            // Not on a tag.
            /**
             * @var array{tag: string, hash: string} $matches
        ...
        $phpVersion = sprintf('%-15s %s', 'Version', PHP_VERSION);
        $longVersion .= PHP_EOL . $phpVersion;

        $phpSapi = sprintf('%-15s %s', 'SAPI', PHP_SAPI);
        $longVersion .= PHP_EOL . $phpSapi;

        $longVersion .= PHP_EOL . PHP_EOL . '<comment>Support PHP Matrix:</>';

        $supportBlock = new FormatterHelper()->formatBlock(
            [
        ...
        $sponsorUrl = sprintf('%1$-15s <href=%2$s>%2$s</>', 'GitHub Sponsor', 'https://github.com/sponsors/tangrufus');
        $longVersion .= PHP_EOL . PHP_EOL . $sponsorUrl;

        $longVersion .= PHP_EOL . PHP_EOL . '<comment>Hire Tang Rufus:</>';

        $hireBlock = new FormatterHelper()->formatBlock(
            [
        ...
        $sponsorUrl = sprintf('%1$-15s <href=%2$s>%2$s</>', 'Contact', 'https://typist.tech/contact/');
        $longVersion .= PHP_EOL . PHP_EOL . $sponsorUrl;
